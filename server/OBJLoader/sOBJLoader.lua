...
OBJLoader.Load = function(path)
	...
	--Debug: print("Server acknowledged request")
...
OBJLoader.Request = function(clientArgs , player)
	-- Check arguments from client.

	--print("sObjectLoader received argument from client for model for ", tostring(clientArgs.player), "model:", clientArgs.path)
	if type(clientArgs.path) ~= "string" then
		return
	end
	-- Try to load the model.
	local model , errorMessage = OBJLoader.Load(clientArgs.path)
	if model == nil then
		warn("[OBJLoader] Could not load "..clientArgs.path..": "..tostring(errorMessage))
		return
	end
	-- Send them the mesh data.
	local args = {
		modelData = model ,
		modelPath = clientArgs.path ,
		playerTarget = clientArgs.player ,
	}
